...
driver_path = Path.cwd() / bundle_dir / "chromedriver"
...
def loadMoviesOnNetflix():
    ...
    # Initiate updateText as empty string
    textTest.set('Value 1')
    ...
    with open(hit_movie_list_path, mode='r') as infile:
        ...
        updateTextString = 'Your expert recommended movie is: '+selectedMovieTitle
        sourceTextString = 'Recommended by: ' + selectedMovieSource

        textTest.set(updateTextString)
...
def updateMovieList(username, password):
    ...
    try:
        ...
        sourceTargetRobertEbert = 'test'
        driver.get(sourceTargetRobertEbert)
    except:
        print('exception 4 thrown')
        try:
            print('trying to remove previous text') 
            sourceText.destroy()
            updateText.destroy()
        except:
            print('no previous text, pass')
            pass
        print('return to orginial exception...')
    ...
    Select(driver.find_element_by_css_selector("select#filters_star_rating_")).select_by_value("4.0")
    ...
    try:
        ...
        print("Exception 5")
        print(e)
        print('destroying previous text...')
        sourceText.destroy()
        updateText.destroy()
...
textTest = StringVar()
updateText = Label(root, text = (textTest), font =( 
            'Verdana', 15), bg= 'black', fg = 'white')
updateText.place(x = 190, y = 600)
updateText.pack()
