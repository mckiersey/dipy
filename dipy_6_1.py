...
movie_list_path = Path.cwd() / bundle_dir / "DipyMoviesOnNetflix.csv"
...
def loadMoviesOnNetflix():
     # Read from file      
    with open(movie_list_path, mode='r') as infile:
        reader = csv.reader(infile)
        readerII = csv.DictReader(open(movie_list_path))
...
def updateMovieList(username, password):
    ...
    # Write to file
    with open(movie_list_path, 'w') as f:  
        w = csv.writer(f)
    ...
    try:
        ...
        updatingText = Label(root, text = ("Invalid Netflix username or password"), font =( 
            'Verdana', 15), bg= 'black', fg = 'white')
        updatingText.place(x = 600, y = 100)
